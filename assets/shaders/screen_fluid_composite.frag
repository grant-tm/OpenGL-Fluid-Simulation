...
void main(void)
{
    ...
    vec3 surface_normal = normalize(vec3(-thickness_gradient.x * 14.0, -thickness_gradient.y * 14.0, 1.0));
    ...
    float rim_light = pow(1.0 - max(dot(surface_normal, view_direction), 0.0), 2.5);
    float specular_light = pow(max(dot(surface_normal, half_vector), 0.0), 18.0);
    float body_factor = clamp(center_thickness * 2.6, 0.0, 1.0);
    ...
    vec3 shaded_color = base_color * (0.40 + diffuse_light * 0.95);
    shaded_color += highlight_color * (specular_light * 0.75 + rim_light * 0.18);

    float alpha_value = clamp(center_thickness * 1.8 + rim_light * 0.10, 0.0, 0.95);
